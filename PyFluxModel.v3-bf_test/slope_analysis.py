...
def analyze_slope(master_data, lgr_data,row,sample_ID,output_folder,r_values,gas_to_read,t_p_data):
	...
	#whilte being right adjusted 15 spaces Micro
	ts = ""
	...
	#plt.savefig(str(output_folder + "\\" + sample_ID + "_" + {r_2_value} + '.png'),dpi = 300, bbox_inches = 'tight')
	...
	master_data.at[row, f"ch4_flux_{r_2_value}"] = flux[0]
	master_data.at[row, f"uncertainty_{r_2_value}"] = flux_error[0]
	master_data.at[row, f"R_value_used_{r_2_value}"] = max(max_r_2)

	print(f"Ended Loop mechanism with threshold = {r_2_value}   |   flux result = {flux[0]}   |   uncertainty = {flux_error[0]}   |   r_value_used = {max(max_r_2)}  \n\n\n")
